...
def forgot_password():
    ...
    print(user)
    if user is not False:
        print('User finns i sjön ')
        senderemail = "skurkeriet1718@gmail.com"
        receiveremail = email
        subject = "Twidder new password"
        senderPassword = "Tengil1337"

        yag = yagmail.SMTP(user=senderemail, password=senderPassword)

        newpass = database_helper.generate_new_pass()
        print(newpass)
        database_helper.reset_password(email, newpass)
        content = ["The password for your twidder account has been reseted, email: " + receiveremail + " Your new password is: " + newpass ]

        yag.send(to=receiveremail, subject=subject, contents =content)

        return jsonify({'success': True, 'message': "Email skickat", 'user': user})
    else:
        print("User finns inte i sjön")
